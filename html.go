...
<div id="app">
    <div class="row">
        <div>
            <textarea name="" id="logList" cols="100" rows="30" readonly="readonly"
                    ...
                    <table>
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="file" value="v0.0.0.1.bin" name="firmware"
                                       id="firmware"/></form>
                            </td>
                        ...
                        <tr>
                            <td>
                                <button class="btn btn-info upgrate" onclick="upload()">上传</button>
                            </td>
                        </tr>
                    </table>
...
function doUpload(file, md5) {
    ...
    form.append('MD5', md5)
    if (fileExt[0] === 'bz2') {
        urlname = '/internal/api/upgrade'
        form.append('firmware', file)
    } else if (fileExt[0] === 'yaml' || fileExt[0] === 'yml') {
        urlname = '/internal/api/config'
    ...
    xhr.onload = function uploadComplete(e) {
        alert('上传成功！请重启')
        console.log('上传成功！', e)
        location.reload()
    } // 添加 上传成功后的回调函数
    xhr.onerror = function uploadFailed(e) {
        console.log('上传失败', e)
    } // 添加 上传失败后的回调函数
...
function uploadFile(file) {
    ...
    fileReader.onload = function (e) {
        ...
        if (currentChunk < chunks) {
            ...
            let md5 = spark.end()
            console.info('md5 value: ', md5)
            box.innerText = md5
            document.getElementsByClassName('MD5').innerHTML = md5
            doUpload(file, md5)
        }
    }
...
function upload() {
    ...
    let fileExt = (/[.]/.exec(filename)) ? /[^.]+$/.exec(filename.toLowerCase()) : ''
    if (!(fileExt[0] === 'bz2' ||
        fileExt[0] === 'yaml' || fileExt[0] === 'yml')) {
        alert('文件仅支持bz2,yaml,yml')
        return
    }
